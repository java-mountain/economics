...
int main()
{
    ...
    std::map<std::string, std::vector<double>> variableCostsAndRevenue; // map for variable monthly costs
    ...
    while(currentProfit<grossProfit) // using while loop until profit going above zero
    {
        ...
        variableCostsAndRevenue.insert(std::pair<std::string, std::vector<double>>(nameOfGoods, {purchaseCost, quantity, sellingPrice}));
    ...
    for(auto itr1=variableCostsAndRevenue.begin(); itr1!=variableCostsAndRevenue.end(); itr1++) //loop with iterator for displaying data of goods
    {
